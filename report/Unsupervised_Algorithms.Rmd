...
abstract: "This document provides an application of unsupervised algorithms in machine learning. For these techniques, the purpose is without the help of an output variable or _supervisor_ to infer the properties of the observations. We will review two methods: k-means and hierarchical clustering. Then we will use some data from Kaggle for observing the application of these techniques. The platform that we use is R, and in some cases, we are going to use a parallel process."
keywords: "Unsupervised, algorithms, k-means, hierarchical, kaggle, R, parallel"
