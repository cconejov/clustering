...
\hypersetup{breaklinks=true,
            ...
            pdfauthor={Cesar Conejo Villalobos (Data Scientist)},
             pdfkeywords = {Unsupervised, algorithms, k-means, hierarchical, kaggle, R, parallel},  
...
\noindent This document provides an application of unsupervised algorithms in
machine learning. For these techniques, the purpose is without the help
of an output variable or \emph{supervisor} to infer the properties of
the observations. We will review two methods: k-means and hierarchical
clustering. Then we will use some data from Kaggle for observing the
application of these techniques. The platform that we use is R, and in
some cases, we are going to use a parallel process.


\vskip 8.5pt \noindent \emph{Keywords}: Unsupervised, algorithms, k-means, hierarchical, kaggle, R, parallel \par
...
\begin{Shaded}
\begin{Highlighting}[]
\CommentTok{#library(stevemisc)}
\CommentTok{#data(uniondensity)}

\CommentTok{#M1 <- lm(union ~ left + size + concen, data=uniondensity)}

\CommentTok{#library(arm)}
\CommentTok{#coefplot(M1)}
\end{Highlighting}
\end{Shaded}
...
\CommentTok{#stargazer(M1, title="A Handsome Table", header=FALSE)}
\end{Highlighting}
\end{Shaded}
